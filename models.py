...
def create_modules(module_defs, img_size):
    ...
    for i, mdef in enumerate(module_defs):
        ...
        # se模块       
            ...
            # extend表示添加一系列对象
            routs.extend([i + l if l < 0 else l for l in layers])  # 当L小于0的时候就是i+L,当L大于0的时候就是L本身。

        elif mdef['type'] == 'shortcut':  # nn.Sequential() placeholder for 'shortcut' layer
            layers = mdef['from']
            filters = output_filters[-1]
            routs.extend([i + l if l < 0 else l for l in layers]) # 群友疑问
...
class Darknet(nn.Module):
    ...
    def forward(self, x, verbose=False): # verbose:冗长
        img_size = x.shape[-2:] # 取出图片什么维度？
        yolo_out, out = [], []
        if verbose:
            str = ''
            print('0', x.shape)
        ...
        for i, (mdef, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            # if mtype in ['convolutional', 'upsample', 'maxpool','se']:
                ...
                x = module(x, out)  # weightedFeatureFusion() 群友疑问
            ...
            # 记录route对应的层
            out.append(x if self.routs[i] else []) # 群友疑问
            if verbose:
        ...
        if self.training:  # train
            ...
            # forward的过程也比较简单，通过得到的module_defs和module_list变量，
            # 通过for循环将整个module_list中的内容进行一遍串联，需要得到的最终结果是YOLO层的输出。
...
def get_yolo_layers(model):
    ...
    # yolo layer的获取是通过解析module_defs这个存储cfg文件中的信息的变量得到的。以yolov3.cfg为例，
    # 最终返回的是yolo层在整个module的序号。比如：第82,94,106个层是YOLO层。

def create_grids(self, img_size=416, ng=(13, 13), device='cpu', type=torch.float32):
    nx, ny = ng  # x and y grid size 网格尺寸
