...
def train():
    ...
    # model = nn.DataParallel(model,devices_ids=devices_ids)
    ...
    nw = min([os.cpu_count(), batch_size if batch_size > 1 else 0, 8])  # number of workers 使用多少个线程加载数据集
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if opt.multi_scale:
                ...
                if sf != 1:
                    ...
                    imgs = F.interpolate(imgs, size=ns, mode='bilinear', align_corners=False) # 双线性插值
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='cfg/yolov3-cbam.cfg', help='*.cfg path')
    parser.add_argument('--data', type=str, default='data2/coco.data', help='*.data path')
    ...
    parser.add_argument('--device', default='', help='device id (cpu)')
    ...
    if not opt.evolve:  # Train normally
        ...
        for _ in range(1):  # generations to evolve 1可以改成其他数字，改成几就是具体的迭代数
